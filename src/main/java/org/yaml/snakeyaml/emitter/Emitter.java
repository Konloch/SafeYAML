...
public final class Emitter implements Emitable
{
	...
	private void expectNode(boolean root, boolean mapping, boolean simpleKey) throws IOException
	{
		...
		else if (event instanceof ScalarEvent || event instanceof CollectionStartEvent)
		{
			...
			{
				if (flowLevel != 0 || canonical || ((SequenceStartEvent) event).isFlow() || checkEmptySequence())
				{
					expectFlowSequence();
				}
				else
			...
			{// MappingStartEvent
				if (flowLevel != 0 || canonical || ((MappingStartEvent) event).isFlow() || checkEmptyMapping())
				{
					expectFlowMapping();
				}
				else
	...
	private boolean checkSimpleKey()
	{
		...
		return length < maxSimpleKeyLength && (event instanceof AliasEvent || (event instanceof ScalarEvent && !analysis.isEmpty() && !analysis.isMultiline()) || checkEmptySequence() || checkEmptyMapping());
	}
