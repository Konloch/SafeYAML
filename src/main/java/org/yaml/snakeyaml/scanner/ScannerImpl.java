...
public final class ScannerImpl implements Scanner
{
	...
	private void fetchMoreTokens()
	{
		...
		{
			throw new YAMLException("The incoming YAML document exceeds the limit: " + loaderOptions.getCodePointLimit() + " code points.");
		}
	...
	private void savePossibleSimpleKey()
	{
		...
		if (this.allowSimpleKey)
		{
			...
			SimpleKey key = new SimpleKey(tokenNumber, required, reader.getIndex(), reader.getLine(), this.reader.getColumn(), this.reader.getMark());
	...
	private boolean checkPlain()
	{
		...
		return Constant.NULL_BL_T_LINEBR.hasNo(c, "-?:,[]{}#&*!|>'\"%@`") || (Constant.NULL_BL_T_LINEBR.hasNo(reader.peek(1)) && (c == '-' || (this.flowLevel == 0 && "?:".indexOf(c) != -1)));
	}
	...
	private String scanDirectiveName(Mark startMark)
	{
		...
		if (length == 0)
		{
			...
			throw new ScannerException("while scanning a directive", startMark, "expected alphabetic or numeric character, but found " + s + "(" + c + ")", reader.getMark());
		}
		String value = reader.prefixForward(length);
		c = reader.peek();
	...
	private List<Integer> scanYamlDirectiveValue(Mark startMark)
	{
		...
		{
			final String s = String.valueOf(Character.toChars(c));
			throw new ScannerException("while scanning a directive", startMark, "expected a digit or '.', but found " + s + "(" + c + ")", reader.getMark());
		}
		reader.forward();
	...
	private Integer scanYamlDirectiveNumber(Mark startMark)
	{
		...
		{
			final String s = String.valueOf(Character.toChars(c));
			throw new ScannerException("while scanning a directive", startMark, "expected a digit, but found " + s + "(" + (c) + ")", reader.getMark());
		}
		int length = 0;
	...
	private String scanTagDirectiveHandle(Mark startMark)
	{
		...
		{
			final String s = String.valueOf(Character.toChars(c));
			throw new ScannerException("while scanning a directive", startMark, "expected ' ', but found " + s + "(" + c + ")", reader.getMark());
		}
		return value;
	}
	...
	private CommentToken scanDirectiveIgnoredLine(Mark startMark)
	{
		...
		{
			final String s = String.valueOf(Character.toChars(c));
			throw new ScannerException("while scanning a directive", startMark, "expected a comment or a line break, but found " + s + "(" + c + ")", reader.getMark());
		}
		return commentToken;
	}
	...
	private Token scanAnchor(boolean isAnchor)
	{
		...
		{
			final String s = String.valueOf(Character.toChars(c));
			throw new ScannerException("while scanning an " + name, startMark, "unexpected character found " + s + "(" + c + ")", reader.getMark());
		}
		String value = reader.prefixForward(length);
		c = reader.peek();
	...
	private Token scanTag()
	{
		...
		if (c == '<')
		{
			...
			if (c != '>')
			{
				...
				final String s = String.valueOf(Character.toChars(c));
				throw new ScannerException("while scanning a tag", startMark, "expected '>', but found '" + s + "' (" + c + ")", reader.getMark());
			}
			reader.forward();
		}
	...
	private Chomping scanBlockScalarIndicators(Mark startMark)
	{
		...
		{
			final String s = String.valueOf(Character.toChars(c));
			throw new ScannerException("while scanning a block scalar", startMark, "expected chomping or indentation indicators, but found " + s + "(" + c + ")", reader.getMark());
		}
	...
	private CommentToken scanBlockScalarIgnoredLine(Mark startMark)
	{
		...
		{
			final String s = String.valueOf(Character.toChars(c));
			throw new ScannerException("while scanning a block scalar", startMark, "expected a comment or a line break, but found " + s + "(" + c + ")", reader.getMark());
		}
		return commentToken;
	}
	...
	private String scanFlowScalarNonSpaces(boolean doubleQuoted, Mark startMark)
	{
		...
		while (true)
		{
			...
			else if (doubleQuoted && c == '\\')
			{
				...
				{
					final String s = String.valueOf(Character.toChars(c));
					throw new ScannerException("while scanning a double-quoted scalar", startMark, "found unknown escape character " + s + "(" + c + ")", reader.getMark());
				}
			}
			else
			{
	...
	private Token scanPlain()
	{
		...
		while (true)
		{
			...
			{
				c = reader.peek(length);
				if (Constant.NULL_BL_T_LINEBR.has(c) || (c == ':' && Constant.NULL_BL_T_LINEBR.has(reader.peek(length + 1), flowLevel != 0 ? ",[]{}" : "")) || (this.flowLevel != 0 && ",?[]{}".indexOf(c) != -1))
				{
					break;
				}
				length++;
	...
	private boolean atEndOfPlain()
	{
		...
		if (reader.peek(wsLength) == '#' || reader.peek(wsLength + 1) == '\0' || this.flowLevel == 0 && wsColumn < this.indent)
		{
			return true;
		}
	...
	private String scanTagHandle(String name, Mark startMark)
	{
		...
		{
			final String s = String.valueOf(Character.toChars(c));
			throw new ScannerException("while scanning a " + name, startMark, "expected '!', but found " + s + "(" + (c) + ")", reader.getMark());
		}
	...
	private String scanUriEscapes(String name, Mark startMark)
	{
		...
		while (reader.peek() == '%')
		{
			...
			catch (NumberFormatException nfe)
			{
				...
				throw new ScannerException("while scanning a " + name, startMark, "expected URI escape sequence of 2 hexadecimal numbers, but found " + s1 + "(" + c1 + ") and " + s2 + "(" + c2 + ")", reader.getMark());
			}
			reader.forward(2);
		}
		buff.flip();
