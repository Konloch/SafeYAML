...
public class SafeConstructor extends BaseConstructor
{
	...
	protected void flattenMapping(MappingNode node, boolean forceStringKeys)
	{
		...
		if (node.isMerged())
		{
			node.setValue(mergeNode(node, true, new HashMap<Object, Integer>(), new ArrayList<NodeTuple>(), forceStringKeys));
		}
	}
	...
	private List<NodeTuple> mergeNode(MappingNode node, boolean isPreffered, Map<Object, Integer> key2index, List<NodeTuple> values, boolean forceStringKeys)
	{
		...
		while (iter.hasNext())
		{
			...
			else
			{
				...
				{ // 1st time merging key
					values.add(nodeTuple);
					// keep track where tuple for the key is
					key2index.put(key, values.size() - 1);
				}
				else if (isPreffered)
				{ // there is value for the key, but we
					// need to override it
	...
	public class ConstructYamlSet implements Construct
	{
		...
		public Object construct(Node node)
		{
			...
			{
				return (constructedObjects.containsKey(node) ? constructedObjects.get(node) : createDefaultSet(((MappingNode) node).getValue().size()));
			}
			else
			{
